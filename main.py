...
def main():
    ...
    while True:
        ...
        if scelta == "1":
            ...
            idx = (
                int(input(f"Seleziona il numero del progetto (1-{len(progetti)}): "))
                - 1
            )
            ...
            while True:
                ...
                risposta = synthesizer.answer(
                    user_query, code_ctx, commit_ctx, analytics_report
                )
        ...
        # --- OPZIONE 3: AGGIORNAMENTO ---
            ...
            q_url = (
                "MATCH (n {project: $p}) WHERE n.url IS NOT NULL RETURN n.url LIMIT 1"
            )
            res = client.execute_query(q_url, {"p": nome})
            url = (
                res[0]["n.url"]
                if res
                else input("URL non trovato. Inseriscilo manualmente: ")
            )
